// Script to create 5 test problems for AI Usecase company
...
const connectDB = async () => {
  try {
    const mongoUri = process.env.MONGO_URI || process.env.MONGODB_URI || 'mongodb://localhost:27017/engineer-connect';
...
const createTestProblems = async () => {
  try {
    await connectDB();
    
    // Find or create AI Usecase user (admin account)
    let aiUser = await User.findOne({ email: 'ai.usecase@engineerconnect.com' });
    
    if (!aiUser) {
      console.log('Creating AI Usecase account...');
      const hashedPassword = await bcrypt.hash('AIUsecase2024!', 10);
      
      aiUser = new User({
        name: 'AI Usecase',
        email: 'ai.usecase@engineerconnect.com',
        username: 'ai_usecase',
        password: hashedPassword,
        role: 'company',
        companyName: 'AI Usecase',
        companyDescription: 'Leading AI solutions provider for engineering challenges'
      });
      await aiUser.save();
      console.log('AI Usecase account created successfully');
    }

    // Delete existing test problems from AI Usecase
    await Problem.deleteMany({ company: 'AI Usecase' });
    ...
    const createdProblems = await Problem.insertMany(problemsWithUser);
    console.log(`Successfully created ${createdProblems.length} test problems`);

    // Display created problems
